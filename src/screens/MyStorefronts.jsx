...
function PreviewAccess({ site }) {
  ...
  return (
    <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px dashed #d9dde6", display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10, fontSize: 12.5, color: "#42505f" }}>
      <span style={{ fontWeight: 600 }}>Preview access:</span>
...
export default function MyStorefronts() {
  ...
  async function load() {
    setError(null);
    try { const s = (await api.myHostedSites()).sites || []; setSites(s); return s; }
    catch (e) { setError(e); return []; }
  }
  useEffect(() => { load(); }, []);

  // Right after creating a store, jump straight into its guided setup (the new
  // draft is not active, so StoreDetail opens in wizard mode automatically).
  async function openNewSite(newId) {
    setCreating(false);
    const s = await load();
    const ns = s.find((x) => x.id === newId);
    if (ns) setSelected(ns);
  }
...
function CreateSiteModal({ onClose, onDone }) {
  ...
  async function submit() {
    ...
    try { const r = await api.createHostedSite(storeName.trim(), slug.trim() || undefined); onDone(r.site?.id); }
  ...
  return (
    <Modal title="New storefront" onClose={onClose}>
      ...
      <div style={{ fontSize: 12.5, color: "#6b7688", marginBottom: 12 }}>
        Starts as a <strong>draft</strong> — set up branding, products and everything else at your own pace.
...
function StoreDetail({ site, onBack, onChanged }) {
  ...
  async function submitForReview() {
    setSubmitting(true);
    try { await api.submitHostedSite(site.id); onChanged?.(); }
  ...
  return (
    <div>
      ...
      {site.status === "draft" && (
        <Card style={{ marginBottom: 18, background: "#f4f6fa", border: "1px solid #dfe4ee" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
            <div style={{ fontSize: 13, color: "#42505f", flex: 1, minWidth: 220 }}>
              This storefront is a <strong>draft</strong> — only people with the preview password can see it. Finish setting it up, then submit it for review.
              An admin approves it before it goes live.
            </div>
            <Btn tone="lime" onClick={submitForReview} disabled={submitting}>
    ...
    <div>
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: "#1b2230" }}>{step.title}</h2>
...
// Final wizard step: pick a plan, submit the draft for admin approval, then pay
// to go live. Adapts to the store's current status so the one panel carries the
// whole journey draft → pending → approved → active.
function GoLivePanel({ site, onChanged }) {
  const [plans, setPlans] = useState(null);
  const [planId, setPlanId] = useState(site.plan_id || null);
  const [invoices, setInvoices] = useState(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState(null);

  useEffect(() => {
    api.publicPlans().then((r) => { const ps = r.plans || []; setPlans(ps); setPlanId((p) => p || ps[0]?.id || null); }).catch(setError);
    api.invoices().then((r) => setInvoices((r.invoices || []).filter((iv) => iv.enrollment_id === site.id))).catch(() => {});
  }, [site.id]);
  ...
  async function submit() {
    if (!planId) { setError(new Error("Pick a plan first")); return; }
    setBusy(true); setError(null);
    try { await api.submitHostedSite(site.id, { plan_id: planId }); onChanged?.(); }
  ...
  return (
    <Card>
      ...
      {site.status === "active" ? (
        <div>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#2e7d32", marginBottom: 4 }}>✓ Your storefront is live</div>
          <div style={{ fontSize: 12.5, color: "#6b7688", marginBottom: 14 }}>
            {site.expiry_date ? `Renews on ${fmtDate(site.expiry_date)}.` : ""} Share your link and start selling.
          </div>
          <a href={previewUrl} target="_blank" rel="noreferrer"><Btn tone="lime">Visit your live site ↗</Btn></a>
        </div>
      ) : (
        <>
          {/* plan picker — editable while it's still a draft */}
          <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 4 }}>Pick a plan</div>
          <div style={{ fontSize: 12.5, color: "#6b7688", marginBottom: 12 }}>Choose the plan your storefront runs on. You'll pay once an admin approves it.</div>
          {!plans ? <Spinner /> : plans.length === 0 ? (
            <div style={{ fontSize: 13, color: "#8a6d2f", marginBottom: 12 }}>No plans available yet — an admin needs to publish one.</div>
          ) : (
          ...
          {/* status-adaptive action */}
          {site.status === "draft" && (
            <Btn tone="lime" disabled={busy || !planId} onClick={submit}>{busy ? "Submitting…" : "Submit for approval"}</Btn>
          )}
          {site.status === "pending" && (
            <div style={{ fontSize: 13, color: "#8a6d2f", background: "#fff7e6", border: "1px solid #f0d9a8", borderRadius: 10, padding: 12 }}>
              ⏳ Submitted — <strong>awaiting admin approval</strong>. You'll get an invoice to pay here the moment it's approved.
            </div>
          )}
          {site.status === "approved" && (
            !invoices ? <Spinner /> : unpaid ? (
              <div style={{ border: "1px solid #eef1f6", borderRadius: 10, padding: 14, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14 }}>{unpaid.invoice_no} · ₹{Number(unpaid.amount).toLocaleString("en-IN")}</div>
                  <div style={{ fontSize: 12, color: "#6b7688", marginTop: 2 }}>Approved — pay to go live.</div>
                </div>
                <Btn tone="lime" disabled={busy} onClick={() => pay(unpaid.id)}>{busy ? "Redirecting…" : "Pay now"}</Btn>
              </div>
            ) : (
              <div style={{ fontSize: 13, color: "#2e7d32" }}>✓ Approved — your invoice is being prepared.</div>
            )
          )}

          <div style={{ marginTop: 14, paddingTop: 12, borderTop: "1px dashed #e6e9f0", fontSize: 12.5 }}>
            <a href={previewUrl} target="_blank" rel="noreferrer" style={{ color: "#3b6fd8", textDecoration: "none" }}>Preview your storefront ↗</a>
            <span style={{ color: "#9aa3b2" }}> — see it before it goes live (uses your preview password).</span>
          </div>
        </>
      )}
    </Card>
...
const CatMapRow = React.memo(function CatMapRow({ siteId, c, onSaved, onError }) {
  ...
  async function save() {
    setBusy(true);
    try { await api.hostedSiteSaveCategoryMap(siteId, c.db_name, c.name, canon.trim()); onSaved(); }
    catch (e) { onError(e); } finally { setBusy(false); }
  }
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr auto auto", gap: 7, alignItems: "center", border: "1px solid #eef1f6", borderRadius: 8, padding: "7px 9px" }}>
      <div style={{ fontSize: 12.5, minWidth: 0 }}>
        <div style={{ color: c.canonical ? "#1b2230" : "#a23a4b", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.name}</div>
        <div style={{ fontSize: 10.5, color: "#b3bccb" }}><span style={{ textTransform: "uppercase" }}>{c.db_name}</span> · {c.count}{c.canonical ? "" : " · unmapped"}</div>
      </div>
...
function CategoryMapPanel({ site }) {
  const [cats, setCats] = useState(null);
  const [q, setQ] = useState("");
  const [error, setError] = useState(null);
  function load() { api.hostedSiteAllCategories(site.id).then((r) => setCats(r.categories || [])).catch(setError); }
  useEffect(() => { load(); }, [site.id]); // eslint-disable-line react-hooks/exhaustive-deps
  const shown = useMemo(() => {
    if (!cats) return null;
    const ql = q.trim().toLowerCase();
    return cats
      .filter((c) => !ql || c.name.toLowerCase().includes(ql) || (c.canonical || "").toLowerCase().includes(ql))
  ...
  return (
    <Card>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
        <div style={{ fontWeight: 700, fontSize: 15 }}>Category grouping</div>
      ...
      <div style={{ position: "relative", marginBottom: 10 }}>
        ...
        <input style={{ ...inputStyle, paddingLeft: 32 }} placeholder="Search categories…" value={q} onChange={(e) => setQ(e.target.value)} />
      </div>
...
function HomepagePresetPanel({ site }) {
            ...
            return (
              <div key={p.id} style={{ border: (previewId ? previewId === p.id : isOn) ? "2px solid #C8FF3D" : "1px solid #e6e9f0", borderRadius: 10, padding: 14, background: "#fff" }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  ...
                  {canPreview && (
                    <Btn small tone="ghost" onClick={() => setPreviewId(previewId === p.id ? null : p.id)}>
      ...
      {previewId && (
        <div style={{ marginTop: 16 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
            <div style={{ fontSize: 12.5, color: "#6b7688" }}>Live preview — <strong>{previewId}</strong> layout (not saved until you click “Use this layout”)</div>
            <a href={previewUrl(previewId)} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "#3b6fd8", display: "inline-flex", alignItems: "center", gap: 4, textDecoration: "none" }}>
              Open in new tab <ExternalLink size={12} />
            </a>
          </div>
          <div style={{ border: "1px solid #e6e9f0", borderRadius: 10, overflow: "hidden", height: 460, background: "#fff" }}>
            <iframe key={previewId} title="Layout preview" src={previewUrl(previewId)} style={{ width: "100%", height: "100%", border: "none" }} />
          </div>
        </div>
      )}
    </Card>
